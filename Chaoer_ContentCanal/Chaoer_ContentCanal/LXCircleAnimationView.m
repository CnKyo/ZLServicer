...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.commentLabel.frame = CGRectMake(self.bgImageView.mleft + self.circelRadius / 4, self.circelRadius / 4, self.circelRadius / 2, self.circelRadius / 2);
...
- (void)initSubView {
    ...
    // 圆形路径
    UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.mwidth / 2, self.mheight / 2)
...
- (void)createAnimationWithStartAngle:(CGFloat)startAngle endAngle:(CGFloat)endAngle { // 光标动画
    ...
    CGPathAddArc(path, NULL, self.mwidth / 2, self.mheight / 2, (self.circelRadius - kMarkerRadius / 2) / 2, startAngle, endAngle, 0);
    pathAnimation.path = path;
    CGPathRelease(path);
    
    self.markerImageView.frame = CGRectMake(-100, self.mheight, kMarkerRadius, kMarkerRadius);
...
- (UIImageView *)markerImageView {
    ...
    if (nil == _markerImageView) {
        ...
        _markerImageView.layer.shadowRadius = 10.f;
